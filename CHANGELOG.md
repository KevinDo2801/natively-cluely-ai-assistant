    ...
    - **Launcher home: hero banner grid replaced with a single calendar link line**: Removes the entire hero section under the Launcher header — the `FeatureSpotlight` "Upcoming features / Support development" auto-rotating banner (left) and the `UpcomingCalendarCard` "Link your calendar / Upcoming events" promo card with its gradient backdrop and meeting peek stack (right) — and replaces it with one plain line: "*Link your calendar* to get notifications for upcoming meetings." The link is rendered by a new self-contained `src/components/ui/LinkCalendarPrompt.tsx`: it stays hidden until the persisted `getCalendarStatus()` check resolves on mount (connected users never see it flash), starts the Google connect flow directly via `calendarConnect()` on click (the same IPC the deleted card's Connect button used), fires `analytics.trackCalendarConnected()` on success, and disappears once connected. Dead code removed with it: `FeatureSpotlight.tsx`, `UpcomingCalendarCard.tsx`, `ConnectCalendarButton.tsx` (its only consumer was the deleted card), the `mainui.png`/`calender.png` banner assets, the Launcher's orphaned `upcomingEvents`/`isCalendarConnected` state and derived vars, the 60 s `getUpcomingEvents` polling loop, and the now-unused i18n keys (`Upcoming features`, `Link your calendar to`, `see upcoming events`, `Connect calendar`, `No upcoming events`, `more`) across all four dictionaries — with new `Link your calendar` / `to get notifications for upcoming meetings.` keys added for ru/zh/ja/es (English source fallback as before). Upcoming-events browsing remains in Settings → Calendar; the meetings history list, refresh button, and calendar sync pipeline are untouched.
