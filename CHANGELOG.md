    ...
    - **Supabase cloud sync now works in packaged builds (previously dev-only)**: `SupabaseAuthService` / `SupabaseSyncService` read `process.env.SUPABASE_URL` / `SUPABASE_ANON_KEY`, but packaged apps have no shell env and `main.ts` only loads `.env` in dev — so the Account tab and cloud sync silently no-oped in every production build. `scripts/build-electron.js` now bakes the (public-by-design) URL + anon key into the main bundle as `__NATIVELY_SUPABASE_URL__` / `__NATIVELY_SUPABASE_ANON_KEY__` (read from the repo `.env` or the build machine's env, with a build-time warning when unset), and a new `electron/services/supabaseConfig.ts` resolves runtime `process.env` first (dev edits take effect without a rebuild) then falls back to the baked values (packaged). The service_role key is never baked. `release-macos.yml` passes `SUPABASE_URL` / `SUPABASE_ANON_KEY` (repo Variables) into the build step so CI releases ship with sync enabled.

    - **Windows installer targets trimmed to x64 only**: the NSIS target previously built both `x64` and `ia32`, but `sqlite-vec` ships only a `windows-x64` binary and `sharp` has no ia32 prebuild, so an ia32 installer would fail or ship a broken native-module stack. `package.json` `build.win.target.nsis.arch` is now `["x64"]` (portable was already x64-only).

    - **New CI release pipeline for Windows** (`.github/workflows/release-windows.yml`): builds the unsigned NSIS installer + portable exe on `windows-latest` (TS7 typecheck, renderer + electron + Rust native module, sqlite-vec + packaged-asset verification), then `--publish onTagOrDraft` attaches `latest.yml` + installers to the GitHub Release on tag builds so the built-in auto-updater resolves, with an Actions-artifact fallback for manual dispatch. Needs **no secrets** (unsigned by design — SmartScreen "Run anyway" is expected); optional `SUPABASE_URL` / `SUPABASE_ANON_KEY` repo Variables get baked into the bundle exactly like the macOS workflow. Runs alongside the existing macOS signed pipeline without racing it (macOS keeps `--publish never`).

    - **Launcher home: hero banner replaced by a calendar link line + restored Upcoming card**: Removes the hero banner grid under the Launcher header — the `FeatureSpotlight` "Upcoming features / Support development" auto-rotating banner (left) and its promotional "Link your calendar" CTA card (right), with all gradients, images, and large visual panels — and replaces that space with one plain line: "*Link your calendar* to get notifications for upcoming meetings." (`src/components/ui/LinkCalendarPrompt.tsx`): it stays hidden until the persisted `getCalendarStatus()` check resolves on mount, starts the Google connect flow directly via `calendarConnect()` on click, fires `analytics.trackCalendarConnected()` on success, and is swapped out the moment a calendar is connected. When connected, the launcher instead shows the restored `UpcomingCalendarCard` peek stack (up to 3 next meetings — time label + attendee avatars, hover reveals the title, "+N more" pill) fed by the restored `getUpcomingEvents()` state and its 60 s polling loop, so users see their calendar again without the marketing banner. Also removed as dead code: `FeatureSpotlight.tsx`, the now-unused `Upcoming features` i18n keys across all four dictionaries, the orphaned `mainui.png` banner asset, and the Launcher's `nextMeeting`/`moreMeetingsCount` dead derived vars. `ConnectCalendarButton.tsx`, `calender.png`, and the calendar i18n keys (`Upcoming events`, `more`, `No upcoming events`, `Link your calendar to`, `see upcoming events`, `Connect calendar`) were restored for the card. Upcoming-events browsing remains in Settings → Calendar; the meetings history list, refresh button, and calendar sync pipeline are untouched.
    ...
    - **Launcher hero: the Upcoming-calendar card now reflows fluidly when the divider is dragged up, instead of being cropped** (`src/components/ui/UpcomingCalendarCard.tsx`, `src/components/Launcher.tsx`, `src/index.css`): the resizable hero panel used to pin its height and clip the 198px card with `overflow: hidden`, so the card's internal top/bottom paddings/margins never adapted and the divider visibly sliced through the content. Now, while a calendar is connected, the pinned hero content becomes a bounded flex column and the card wrapper (`flex-1 min-h-0`) receives the real leftover height; the card root is a size query container (`.cc-card`, `container-type: size`) and every vertical rhythm value inside was converted from fixed Tailwind spacing to `clamp(px, Xcqh, px)` units (`1cqh` = 1% of the card's current height; at the designed 198px open state each value reproduces the original spacing within ~0.5px, so the default look is unchanged). Dragging the divider up now continuously tightens paddings/margins (row gap, row padding/min-height, hero margins, accent-bar inset) with no per-frame JS measurement, and a `@container` cascade sheds optional content at heights where it no longer fits — 3rd side row ≤150px, "+N more" hint ≤122px, description ≤116px, side column ≤100px, Join pill ≤92px, title ≤64px, body ≤52px — so the card collapses to a clean time-only state rather than clipping. `measureHeroBounds` now derives the connected panel's natural full height from the designed 198px block (header row + 24px gap + 198px + section padding) instead of reading a DOM offsetHeight that would report the shrunken flex leftover and feed a moving target back into the drag clamp.
